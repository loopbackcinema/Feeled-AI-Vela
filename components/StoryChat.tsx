...
const StoryChat: React.FC<StoryChatProps> = ({ story }) => {
    ...
    return (
        <div className="bg-white rounded-[3.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] border-4 border-white ring-2 ring-indigo-50 overflow-hidden flex flex-col h-[650px] transform hover:scale-[1.01] transition-transform duration-500">
            {/* Premium Header */}
            ...
            <div className="flex-grow overflow-y-auto p-8 bg-slate-50/50 space-y-6">
                {messages.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full text-slate-400 space-y-6 px-10 text-center">
                        <div className="w-24 h-24 bg-white rounded-[2rem] shadow-xl flex items-center justify-center text-5xl animate-float">💬</div>
                        <div className="space-y-2">
                             <p className="text-2xl font-black text-slate-800 leading-tight">Have doubts about the story?</p>
                             <p className="text-lg font-bold text-slate-400">Ask me anything about the concept or Maya's journey!</p>
                        </div>
                    </div>
                )}
            ...
            <div className="p-6 bg-white border-t-2 border-slate-50 relative z-10">
                <form onSubmit={handleSend} className="flex gap-4 p-2 bg-slate-50 rounded-[2.5rem] border-2 border-transparent focus-within:border-indigo-200 transition-all shadow-inner">
                    <input
                        type="text"
                        value={input}
                        onChange={(e) => setInput(e.target.value)}
                        placeholder="Ask me a question about the lesson..."
                    ...
                    <button 
                        ...
                        className="bg-indigo-600 text-white w-16 h-16 rounded-[1.5rem] flex items-center justify-center hover:bg-indigo-700 transition-all disabled:opacity-50 shadow-2xl shadow-indigo-200 group active:scale-90"
                    >
