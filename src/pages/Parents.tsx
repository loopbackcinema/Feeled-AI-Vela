        ...
        <div className="w-full max-w-5xl mx-auto space-y-24 animate-fade-in py-12 px-6">
            ...
            <div className="grid lg:grid-cols-2 gap-12">
                <div className="bg-slate-900 text-white p-16 rounded-[4rem] shadow-4xl space-y-10 relative overflow-hidden">
                    ...
                    <ul className="space-y-6 relative z-10 font-black text-indigo-200">
                    ...
                    <div className="p-8 bg-white/10 rounded-3xl border border-white/20 relative z-10">
                        <p className="text-slate-400 font-bold italic">"It supports learning by adjusting how a lesson is presented, not who your child is."</p>
                    </div>
                </div>
                ...
                <div className="bg-white p-16 rounded-[4rem] border-2 border-slate-100 shadow-sm space-y-10 flex flex-col justify-center">
                    ...
                    <div className="flex flex-wrap gap-4">
                        {["Anxiety", "Curiosity", "Fear of failure", "Confidence", "Fatigue", "Excitement"].map(tag => (
                            <span key={tag} className="px-6 py-2 bg-slate-50 text-slate-800 font-black text-sm rounded-full border border-slate-100">{tag}</span>
                        ))}
                    </div>
                    <p className="text-slate-400 font-bold leading-relaxed pt-6">Traditional systems often ignore these realities. FeelEd AI explores whether learning can slow down when a child feels overwhelmed or encourage gently when confidence is low.</p>
                </div>
            </div>
            ...
            <div className="bg-emerald-50 border-4 border-white p-12 md:p-24 rounded-[4rem] shadow-2xl space-y-12">
                ...
                <div className="p-8 bg-white rounded-[3rem] border border-emerald-100 text-center">
                    <p className="text-2xl font-black text-emerald-900 uppercase tracking-widest italic">Your child is never reduced to data.</p>
                </div>
            </div>

            {/* Protection Principles */}
            <div className="bg-white p-12 md:p-20 rounded-[4rem] border border-slate-200 shadow-sm space-y-12">
                <h3 className="text-3xl font-black text-slate-900">How Your Child Is Protected</h3>
                <div className="grid md:grid-cols-2 gap-16 items-center">
                    <div className="space-y-8">
                        <p className="text-lg font-bold text-slate-500 leading-relaxed uppercase tracking-widest text-[10px]">Strict Principles</p>
                    ...
                    <div className="bg-slate-50 p-12 rounded-[4rem] border border-slate-100 space-y-4 text-center">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {["Temporary", "Contextual", "Teacher-guided", "Child-first"].map(val => (
                                <div key={val} className="bg-white p-4 rounded-2xl shadow-sm font-black text-slate-800 border border-slate-100">{val}</div>
                            ))}
                        </div>
                    </div>
                </div>
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                <div className="bg-indigo-50 p-16 rounded-[4rem] border-2 border-indigo-100 shadow-sm space-y-10">
                    <h3 className="text-3xl font-black text-indigo-900">Who Is This Designed For?</h3>
                    <div className="grid grid-cols-1 gap-4 font-black text-indigo-700">
                ...
                <div className="bg-white p-16 rounded-[4rem] border border-slate-100 shadow-sm space-y-10 flex flex-col justify-center text-center">
                    <p className="text-2xl font-black text-slate-500 italic leading-relaxed">
