...
const StoryDisplay: React.FC<StoryDisplayProps> = ({ story, base64Audio, isAudioLoading, onTryAnother }) => {
    ...
    return (
        <div className="w-full max-w-3xl bg-white p-6 md:p-8 rounded-2xl shadow-lg border border-slate-200 animate-fade-in">
            ...
            <p className="text-center text-slate-500 mb-6 italic">Emotion Tone: {story.emotion_tone}</p>

            <div className="my-6 text-center">
                 <button 
                    onClick={handlePlayPause} 
                    disabled={isDecoding || isAudioLoading || !base64Audio}
                ...
                >
                    {(isDecoding || isAudioLoading) ? <Spinner /> : (
                    ...
                    <span>
                        {isAudioLoading ? 'Generating Audio...' : 
                         isDecoding ? 'Preparing Audio...' : 
                         isPlaying ? 'Stop Story' : 'Play Story as Voice'}
                    </span>
                 </button>
            </div>
            ...
            <div className="mt-8 border-t border-slate-200 pt-6">
                ...
                <div className="flex flex-col md:flex-row justify-center items-center gap-4">
                    <button onClick={handleDownloadAudio} disabled={!base64Audio} className="flex items-center justify-center gap-2 w-full md:w-auto text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg px-4 py-2 transition-colors disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed">
