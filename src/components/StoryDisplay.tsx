            ...
            <div className="space-y-6">
                {quiz.map((q, index) => (
                    <div key={index} className="bg-white p-5 rounded-2xl shadow-sm border border-indigo-50 transition-all hover:shadow-md">
                        ...
                        {showResult[index] && (
                             <div className={`mt-4 p-3 rounded-lg text-sm font-semibold text-center border ${answers[index] === q.answer ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-800 border-red-200'}`}>
...
const StoryDisplay: React.FC<StoryDisplayProps> = ({ story, base64Audio, isAudioLoading, base64Image, imageMimeType, isImageLoading, onTryAnother }) => {
    ...
    const generateCertificate = async (e: React.FormEvent) => {
        ...
        ctx.fillText('INSTITUTE OF EMOTION ADAPTIVE EDUCATION', 600, 85);

        // Left Logo: Education Symbol Seal
        ctx.save();
        ...
        // Right Logo: FeelEd AI Logo
        const logoPromise = new Promise<void>((resolve) => {
            const img = new Image();
            img.src = '/logo.svg';
            img.onload = () => {
                const logoSize = 130;
                ctx.drawImage(img, 910, 115, logoSize, logoSize);
                resolve();
            };
            img.onerror = () => { resolve(); };
        });
        ...
        // Concept with Word Wrap
        ctx.font = 'italic 25px "Nunito", sans-serif';
        ctx.fillStyle = '#4b5563';
        ctx.fillText('For successfully mastering the topic:', 600, 510);
        ctx.font = 'bold 32px "Nunito", sans-serif';
        ctx.fillStyle = '#1e3a8a';
        wrapText(ctx, story.title, 600, 560, 800, 40);
        ...
        ctx.fillText('FeelEd AI', 600, 695);
        ctx.font = '20px "Nunito", sans-serif';
        ctx.fillStyle = '#94a3b8';
        ctx.fillText('Emotion-Adaptive Education Framework', 600, 725);
        ctx.font = 'bold 22px "Nunito", sans-serif';
        ctx.fillStyle = '#475569';
        ctx.fillText('https://feeledai.com', 600, 765);
        ...
        for(let i=0; i<8; i++) {
            for(let j=0; j<8; j++) {
                if (Math.random() > 0.4) ctx.fillRect(qrX + i*10, qrY + j*10, 10, 10);
            }
        }
    ...
    const handleCertificateShare = async () => {
        ...
        if (navigator.share) {
            try {
                ...
                const shareData: ShareData = { files: [file], text: MARKETING_TEXT };
    ...
    const handleSocialShare = (platform: 'whatsapp' | 'facebook' | 'twitter') => {
        ...
        let shareUrl = platform === 'whatsapp' ? `https://wa.me/?text=${text}` : platform === 'facebook' ? `https://www.facebook.com/sharer/sharer.php?u=${url}&quote=${text}` : `https://twitter.com/intent/tweet?text=${text}`;
    ...
    const getStoryAsText = useCallback(() => `*${story.title}*\n\n*Introduction:*\n${story.introduction}\n\n*Concept Explanation:*\n${story.concept_explanation}\n\n- Generated by FeelEd AI (https://feeledai.com)`, [story]);
    ...
    const handleSmartShare = async () => {
        ...
        const shareData: ShareData = { title: story.title, text: getStoryAsText() };
        if (base64Image && imageMimeType) {
            try {
                const res = await fetch(`data:${imageMimeType};base64,${base64Image}`);
                const file = new File([await res.blob()], "story_image.png", { type: imageMimeType });
    ...
    const handleCopyStory = () => {
        navigator.clipboard.writeText(getStoryAsText()).then(() => {
            setCopySuccess(true);
            setTimeout(() => setCopySuccess(false), 2000);
        });
    };

    const handleShareWhatsApp = () => window.open(`https://wa.me/?text=${encodeURIComponent(getStoryAsText())}`, '_blank', 'noopener,noreferrer');
        ...
        <div className="w-full max-w-4xl mx-auto glass-panel p-6 md:p-10 rounded-3xl shadow-2xl border border-white/40">
                ...
                >
                    {isAudioLoading || isDecoding ? <Spinner /> : (
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6">
                            {isPlaying ? <path d="M5.25 7.5A2.25 2.25 0 0 1 7.5 5.25h9a2.25 2.25 0 0 1 2.25 2.25v9a2.25 2.25 0 0 1-2.25 2.25h-9a2.25 2.25 0 0 1-2.25-2.25v-9Z" /> : <path d="M5.25 5.25v13.5l13.5-6.75L5.25 5.25Z" />}
                        </svg>
                    )}
                    <span className="text-lg">{isAudioLoading ? 'Generating Voice...' : isDecoding ? 'Preparing Audio...' : isPlaying ? 'Stop Story' : 'Play Story as Voice'}</span>
                 </button>
            </div>
                    ...
                    {!certificateBlob ? (
                        <>
                            ...
                            <form onSubmit={generateCertificate} className="max-w-md mx-auto space-y-4 relative z-10">
                                <input type="text" placeholder="Student Name" value={studentName} onChange={(e) => setStudentName(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-amber-200 focus:ring-2 focus:ring-amber-400 outline-none" required />
                                <input type="text" placeholder="Class / Grade (e.g., 5th Std)" value={studentClass} onChange={(e) => setStudentClass(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-amber-200 focus:ring-2 focus:ring-amber-400 outline-none" required />
                                <input type="text" placeholder="School Name" value={schoolName} onChange={(e) => setSchoolName(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-amber-200 focus:ring-2 focus:ring-amber-400 outline-none" required />
                                <button type="submit" disabled={generatingCert} className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 rounded-xl shadow-lg transition-transform transform hover:scale-105 disabled:opacity-70">{generatingCert ? 'Generating...' : 'Get Certificate 🏆'}</button>
                            </form>
                        </>
                    ) : (
                        <div className="space-y-6">
                            <h3 className="text-2xl font-bold text-amber-700">🎓 Your Certificate is Ready!</h3>
                            <img src={URL.createObjectURL(certificateBlob)} alt="Certificate" className="w-full rounded-xl shadow-lg border-4 border-white mx-auto max-w-lg" />
                            <div className="flex justify-center">
                                <button onClick={handleCertificateShare} className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold py-3 px-10 rounded-xl shadow-lg flex items-center justify-center gap-3 transition-transform transform hover:scale-105 w-full md:w-auto">
                            ...
                            <div className="pt-4 border-t border-amber-200">
                                ...
                                <div className="flex justify-center gap-4">
                                    <button onClick={() => handleSocialShare('whatsapp')} className="bg-[#25D366] text-white p-3 rounded-full shadow hover:scale-110 transition"><svg className="w-6 h-6" fill="currentColor" viewBox="0 0 448 512"><path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.8 0-67.6-9.5-97.8-26.7l-7.1-4.2-73.3 19.3 19.3-71.6-4.7-7.5c-19.1-30.3-29.8-66-29.8-103.3 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"/></svg></button>
                                    <button onClick={() => handleSocialShare('facebook')} className="bg-[#1877F2] text-white p-3 rounded-full shadow hover:scale-110 transition"><svg className="w-6 h-6" fill="currentColor" viewBox="0 0 512 512"><path d="M504 256C504 119 393 8 256 8S8 119 8 256c0 123.78 90.69 226.38 209.25 245V327.69h-63V256h63v-54.64c0-62.15 37-96.48 93.67-96.48 27.14 0 55.52 4.84 55.52 4.84v61h-31.28c-30.8 0-40.41 19.12-40.41 38.73V256h68.78l-11 71.69h-57.78V501C413.31 482.38 504 379.78 504 256z"/></svg></button>
                                    <button onClick={() => handleSocialShare('twitter')} className="bg-black text-white p-3 rounded-full shadow hover:scale-110 transition"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 512 512"><path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM365.7 444h33.5L110.5 80.9H74.5L365.7 444z"/></svg></button>
                                </div>
                            </div>
                            <button onClick={() => setShowCertificateForm(false)} className="text-xs text-amber-500 hover:underline">Close</button>
                        </div>
                    )}
                </div>
            )}
            
            <div ref={chatSectionRef}><StoryChat story={story} /></div>
            ...
            <div className="mt-12 bg-white/50 rounded-2xl p-6 backdrop-blur-sm border border-white/20">
                ...
                <div className="flex flex-wrap justify-center gap-4">
                     <button onClick={handleDownloadAudio} disabled={!base64Audio} className="flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-semibold py-2 px-4 rounded-xl shadow-sm transition-colors disabled:opacity-50"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>Download Audio</button>
                    <button onClick={scrollToChat} className="flex items-center justify-center gap-2 bg-indigo-600 text-white hover:bg-indigo-700 font-semibold py-2 px-4 rounded-xl shadow-md transition-all transform hover:scale-105"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" /></svg>Discuss</button>
                    {canShare && <button onClick={handleSmartShare} className="flex items-center justify-center gap-2 bg-pink-600 text-white hover:bg-pink-700 font-semibold py-2 px-4 rounded-xl shadow-sm transition-colors"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 1 0 0 2.186m0-2.186c.19.026.37.06.55.098a2.25 2.25 0 0 1 1.455 1.455c.038.18.072.36.098.55m0 0a2.25 2.25 0 0 0 2.186 0m0-2.186a2.25 2.25 0 0 0 0-2.186m0 2.186c-.19-.026-.37-.06-.55-.098a2.25 2.25 0 0 1-1.455-1.455c-.038-.18-.072-.36-.098-.55m0 0a2.25 2.25 0 0 0-2.186 0m0 2.186.001.001" /></svg>Share Image + Story</button>}
                    <button onClick={handleCopyStory} className="flex items-center justify-center gap-2 bg-slate-100 text-slate-700 hover:bg-slate-200 font-semibold py-2 px-4 rounded-xl shadow-sm transition-colors">{copySuccess ? <><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-green-600"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>Copied!</> : <><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125V7.875c0-.621.504-1.125 1.125-1.125H6.75a9.06 9.06 0 0 1 1.5.124m7.5 10.376h3.375c.621 0 1.125-.504 1.125-1.125V11.25c0-4.46-3.243-8.161-7.5-8.876a9.06 9.06 0 0 0-1.5-.124H9.375c-.621 0-1.125.504-1.125 1.125v3.5m7.5 10.375H9.375a1.125 1.125 0 0 1-1.125-1.125v-9.25m12 6.625v-1.875a3.375 3.375 0 0 0-3.375-3.375h-1.5" /></svg>Copy Story</>}</button>
                    <button onClick={handleShareWhatsApp} className="flex items-center justify-center gap-2 bg-[#25D366] text-white hover:bg-[#20bd5a] font-semibold py-2 px-4 rounded-xl shadow-sm transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="currentColor" viewBox="0 0 448 512"><path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.8 0-67.6-9.5-97.8-26.7l-7.1-4.2-73.3 19.3 19.3-71.6-4.7-7.5c-19.1-30.3-29.8-66-29.8-103.3 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"/></svg>WhatsApp</button>
                </div>
            </div>

            <div className="mt-12 text-center">
                <button onClick={onTryAnother} className="bg-slate-800 text-white font-bold py-3 px-8 rounded-full hover:bg-slate-900 focus:outline-none focus:ring-4 focus:ring-slate-400 transition-transform transform hover:scale-105 shadow-lg">✨ Create Another Story</button>
            </div>
        </div>
    );
};
