    ...
    return (
        <header className="w-full bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-slate-100">
            <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                ...
                <nav className="hidden md:flex items-center space-x-1 p-1 bg-slate-50 rounded-xl border border-slate-100">
                    <button onClick={() => onNavigate('about')} className="text-slate-600 hover:text-blue-600 hover:bg-white px-3 py-1.5 rounded-lg transition-all text-sm font-semibold">About</button>
                    <button onClick={() => onNavigate('founder')} className="text-slate-600 hover:text-blue-600 hover:bg-white px-3 py-1.5 rounded-lg transition-all text-sm font-semibold">Founder</button>
                    <button onClick={() => onNavigate('research')} className="text-slate-600 hover:text-blue-600 hover:bg-white px-3 py-1.5 rounded-lg transition-all text-sm font-semibold">Research</button>
                    <button onClick={() => onNavigate('contact')} className="text-slate-600 hover:text-blue-600 hover:bg-white px-3 py-1.5 rounded-lg transition-all text-sm font-semibold">Contact</button>
                </nav>
                <button
                    onClick={handleFeedbackClick}
                    aria-label="Provide feedback via WhatsApp"
                    className="flex items-center gap-2 bg-green-500 text-white font-bold py-2 px-4 rounded-full hover:bg-green-600 focus:outline-none focus:ring-4 focus:ring-green-200 transition-all transform hover:scale-105 shadow-md hover:shadow-lg"
                >
