    ...
    return (
        <header className="w-full bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex justify-between items-center">
                {/* Brand */}
                <button onClick={() => handleLinkClick('dashboard')} className="flex items-center gap-3 group">
                ...
                <nav className="hidden lg:flex items-center gap-4">
                    <button 
                        ...
                        className="bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 text-white px-8 py-3.5 rounded-full font-black uppercase tracking-widest text-[11px] shadow-2xl hover:scale-110 transition-all flex items-center gap-3 ring-4 ring-pink-50"
                    >
            ...
            {/* Mobile Menu */}
            {isMenuOpen && (
                <div className="lg:hidden bg-white border-t border-slate-100 animate-fade-in absolute w-full shadow-2xl z-50 overflow-hidden">
                    <nav className="flex flex-col p-6 space-y-4">
                        <button 
                            onClick={() => handleLinkClick('dashboard')}
                            className="w-full text-left px-8 py-8 rounded-[2.5rem] bg-gradient-to-r from-pink-500 to-indigo-600 text-white flex items-center justify-between shadow-2xl"
                        >
                            <span className="font-black text-2xl uppercase tracking-tighter">Enter Student Hub</span>
                            <span className="text-4xl">🚀</span>
                        </button>
                        <div className="grid grid-cols-2 gap-4">
                            <button onClick={() => handleLinkClick('generator')} className="p-6 rounded-3xl bg-slate-50 text-slate-700 font-black text-xs uppercase tracking-widest text-center border border-slate-100">Teachers</button>
                            <button onClick={() => handleLinkClick('parents')} className="p-6 rounded-3xl bg-slate-50 text-slate-700 font-black text-xs uppercase tracking-widest text-center border border-slate-100">Parents</button>
                        </div>
                        <button onClick={() => handleLinkClick('research')} className="w-full text-center py-5 font-black text-slate-500 uppercase tracking-widest text-[10px]">Scientific Portfolio</button>
                        <button onClick={() => handleLinkClick('contact')} className="w-full bg-slate-900 text-white py-6 rounded-[2rem] font-black uppercase tracking-widest text-xs">Contact Us</button>
                    </nav>
                </div>
            )}
        </header>
