...
const Header: React.FC<HeaderProps> = ({ onNavigate }) => {
    ...
    const navItems = [
        ...
        { id: 'research-pilot', label: 'Voice Pilot 🎙️' },
                ...
                <nav className="hidden lg:flex items-center space-x-1">
                    {navItems.map((item) => (
                        <button 
                            ...
                            className="px-3 py-2 rounded-lg text-[11px] font-black text-slate-500 hover:text-indigo-600 hover:bg-slate-50 transition-all uppercase tracking-[0.1em]"
                        >
                            {item.label}
                        </button>
                    ))}
