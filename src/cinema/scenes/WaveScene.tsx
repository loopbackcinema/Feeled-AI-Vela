...
const WaveScene: React.FC<SceneRenderProps> = ({ tick, color, isFrozen, isReveal }) => {
    const t = isFrozen ? 0 : tick * 0.05;
    const pts1 = Array.from({length:24},(_,j) => `${22+j*15},${112+Math.sin(j*0.52+t*1.2)*32}`).join(' ');
    const pts2 = Array.from({length:24},(_,j) => `${22+j*15},${162+Math.sin(j*0.52+t*1.2+Math.PI)*20}`).join(' ');
    return (<>
        ...
        {isReveal && (<><rect x="132" y="216" width="136" height="24" rx="7" fill="rgba(0,0,10,.85)" stroke="#34d399" strokeWidth="1.2" strokeOpacity="0.5"/>
        <text x="200" y="232" fill="#86efac" fontSize="11" textAnchor="middle" fontFamily="ui-monospace,monospace">v = fλ</text></>)}
    </>);
};
